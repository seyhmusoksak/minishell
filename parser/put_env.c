...
/*   Updated: 2024/07/21 17:33:23 by mehmyilm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// TEST METNİ:
//  "e"c"h"o '$USER' "a'm'a bu kez $USER.dayii $" | ls -a "$PWD$USER$ bir de '$SHELL'" $PWD.123sa "$USER" | c"a"t "a"."txt b'.'txt" "$USER$DAYI$PWD" | "Deneyin $USER sizde" '$PWD'
...
char	*ft_dolar_handler(char *str, t_dolar *dolar, t_parser *prs, t_env *env)
{
	...
	while (str[prs->d])
	{
		if (str[prs->d] == '$' && ft_isdolr(str, prs->d))
